...
The agent workflow generates the three input files GenPipes RNA-seq Light requires: a readset file, a design file, and INI config files. Each is produced by a dedicated agent. Only the readset agent is implemented so far.
...
agents/
  readset_agent.py        ← system prompt and config for readset generation
  # design_agent.py       ← coming soon
...
`setup.py` creates persistent agent definitions on Anthropic's servers and saves their IDs to `agent_config.json`. Session runners (`generate_readset.py` etc.) open a conversation with the relevant agent, send file listings and metadata, and receive the output file.

### Step 1 — Register agents

Run once. Re-run only if `agent_config.json` is deleted or you want to update the agent definition.
